...
export class NormalizationService {
  ...
  private restructureNestedAttributes(v: StrapiEntry, nestedAttribute?: string[]) {
    if (nestedAttribute) {
      nestedAttribute.map((attribute) => {
        if (v.attributes[attribute].data){
          v.attributes[attribute] = v.attributes[attribute].data.map((nv: StrapiEntry) => ({ id: nv.id, ...nv.attributes }));
        }
      });
    }
